package com.kbj.androxlsxparser.mxlsxparser;

import android.content.Context;

import com.kbj.androxlsxparser.mxlsxparser.exceptions.MissingSheetException;
import com.kbj.androxlsxparser.mxlsxparser.impl.StreamingSheetReader;
import com.kbj.androxlsxparser.mxlsxparser.impl.StreamingWorkbook;
import com.kbj.androxlsxparser.mxlsxparser.impl.StreamingWorkbookReader;
import com.kbj.androxlsxparser.mxlsxparser.sst.BufferedStringsTable;
import com.kbj.androxlsxparser.mxlsxparser.exceptions.CloseException;
import com.kbj.androxlsxparser.mxlsxparser.exceptions.OpenException;
import com.kbj.androxlsxparser.mxlsxparser.exceptions.ReadException;
...
import static com.kbj.androxlsxparser.mxlsxparser.XmlUtils.document;
import static com.kbj.androxlsxparser.mxlsxparser.XmlUtils.searchForNodeList;
import static com.kbj.androxlsxparser.mxlsxparser.impl.TempFileUtil.writeInputStreamToFile;
...
public class StreamingReader implements Iterable<Row>, AutoCloseable {
  ...
  public static class Builder {
     ...
     */
    public Workbook open(Context context, InputStream is) {
      StreamingWorkbookReader workbook = new StreamingWorkbookReader(this);
      workbook.init(context, is);
     ...
     */
    public Workbook open(Context context, File file) {
      StreamingWorkbookReader workbook = new StreamingWorkbookReader(this);
      workbook.init(context, file);
      return new StreamingWorkbook(workbook);
    }
     ...
     * @return built streaming reader instance
     * @throws ReadException if there is an issue reading the stream
     * @deprecated This method will be removed in a future release. Use {@link Builder#open(Context,InputStream)} instead
     */
    public StreamingReader read(Context context, InputStream is) {
      File f = null;
      try {
        f = writeInputStreamToFile(context, is, bufferSize);
        log.debug("Created temp file [" + f.getAbsolutePath() + "]");

        StreamingReader r = read(context, f);
        r.tmp = f;
        return r;
      } catch(IOException e) {
     ...
     * @throws OpenException if there is an issue opening the file
     * @throws ReadException if there is an issue reading the file
     * @deprecated This method will be removed in a future release. Use {@link Builder#open(Context, File)} instead
     */
    public StreamingReader read(Context context, File f) {
      try {
        OPCPackage pkg;
        if(password != null) {
          // Based on: https://poi.apache.org/encryption.html
          POIFSFileSystem poifs = new POIFSFileSystem(new FileInputStream(f));
        ...
        File sstCache = null;
        if(sstCacheSize > 0) {
          sstCache = File.createTempFile("", "", context.getCacheDir());
