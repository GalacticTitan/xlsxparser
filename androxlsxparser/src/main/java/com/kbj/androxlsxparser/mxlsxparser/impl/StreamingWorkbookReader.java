...
public class StreamingWorkbookReader implements Iterable<Sheet>, AutoCloseable {
   ...
   */
  @Deprecated
  public StreamingWorkbookReader(SharedStringsTable sst, File sstCache, OPCPackage pkg, StreamingSheetReader reader, StreamingReader.Builder builder) {
    this.sst = sst;
  ...
  public void init(Context context, InputStream is) {
    File f = null;
    try {
      f = writeInputStreamToFile(context, is, builder.getBufferSize());
  ...
  public void init(Context context, File f) {
    try {
